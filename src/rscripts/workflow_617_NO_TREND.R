# ============================================================================
# WORKFLOW 617 NO TREND - BASELINE SIN FEATURES DE TENDENCIA
# ============================================================================
#
# FILOSOFÍA:
# - Lógica IDÉNTICA a z610_WorkFlow_01_gerencial_julio.ipynb
# - Incluye: lags 1,2,3,6 + deltas + rolling stats + ratios + volatilidad
...
EXP_DIR <- file.path(BASE_DIR, "exp", "exp_no_trend")

dir.create(EXP_DIR, showWarnings = FALSE, recursive = TRUE)

LOG_FILE <- file.path(EXP_DIR, paste0("workflow_no_trend_", LOG_TIMESTAMP, ".log"))
...
cat("\n")
cat("========================================\n")
cat(" WORKFLOW 617 NO TREND (z610 logic)\n")
cat("   Paralelo + Caching + Logging\n")
cat("========================================\n\n")

log_msg(paste(rep("=", 60), collapse = ""))
log_msg("WORKFLOW 617 NO TREND INICIADO")
log_msg(paste(rep("=", 60), collapse = ""))
...
PARAM_GLOBAL <- list()
PARAM_GLOBAL$experimento <- "WF617"
...
ejecutar_semilla <- function(seed_idx, semilla, PARAM_GLOBAL, BASE_DIR, DATASETS_DIR,
  ...
  tryCatch({
    ...
    # Crear directorio
    exp_folder <- paste0("WF617", seed_idx - 1, "_seed", seed_idx, "_NO_TREND")
    ...
    if (file.exists(dataset_cache_file)) {
      ...
      # === TENDENCIAS (ventanas 3, 6) - ELIMINADAS EN WF617 ===
      # NOTA: En WF616 se generaban ~58 features de trend (slope lineal)
      # Estas features contribuyen con 61.2% de la ganancia del modelo
      # En este experimento las EXCLUIMOS para medir su impacto específico
      seed_log("SKIPPING TREND FEATURES (excluded in WF617)", level = "WARNING")
...
cat("\n⚡ TODAS LAS", length(PARAM_GLOBAL$semillas), "SEMILLAS CORRIENDO EN NO TREND MODE ⚡\n\n")
...
# Guardar resumen
fwrite(resumen_df, file.path(EXP_DIR, "resumen_no_trend_exp6170.txt"), sep = "\t")
saveRDS(resultados, file.path(EXP_DIR, "resultados_no_trend_exp6170.rds"))

log_msg("Resumen guardado en: resumen_no_trend_exp6170.txt", level = "SUCCESS")
...
log_msg("WORKFLOW NO TREND FINALIZADO EXITOSAMENTE", level = "SUCCESS")
